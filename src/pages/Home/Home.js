...
const Home = (props) => {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await axiosSecured(`/api/products`);
      // console.log(data);
      if (data?.success) {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await axios.get(
        "https://chitropot-server.vercel.app/products"
      );
            ...
            <ul className='list list-inline ml-auto filterTab mb-0 res-full'>
              {allCategory?.map((category, index) => (
                <li className='list list-inline-item' key={index}>
                  <button
                    className={`cursor btn text-capitalize ${
                      category?.categoryName === currentCategory && "act"
                    }`}
